...
# Get columns connections data
column_connections = pd.read_excel(os.path.join('static', 'column_connections.xlsx'))

# Store distinct workbook/worksheet into a dataframe
worksheets = column_connections[['workbook', 'worksheet']].groupby(['workbook', 'worksheet']).first().reset_index()
...
# Iterate over dataframe using the itertuples method (Use Pandas docummentation as reference)
